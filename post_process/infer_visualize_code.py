    ...
    for frame in tqdm(range(start, end)):
        ...
        for bbox_id in range(num_bbox):
            ...
            with torch.no_grad():
                ...
                # =====================================================================
                # ========= ADD THIS BLOCK to visualize the output parameters =========
                # =====================================================================
                print("\n" + "="*50)
                print(f"MODEL OUTPUT PARAMETERS FOR FRAME {frame}")
                print("="*50)
                ...
                for key, value in out.items():
                    if isinstance(value, torch.Tensor):
                        ...
                        print(f"  Value (first 5 elements if large):\n{numpy_value.flatten()[:5]}")
                    elif isinstance(value, dict):
                        print(f"\n--- Parameter Dictionary: '{key}' ---")
                        for sub_key, sub_value in value.items():
                             if isinstance(sub_value, torch.Tensor):
                                print(f"  --- Sub-Parameter: '{sub_key}' ---")
                                print(f"    Shape: {sub_value.shape}")
                                numpy_sub_value = sub_value.detach().cpu().numpy()
                                print(f"    Value (first 5 elements if large):\n{numpy_sub_value.flatten()[:5]}")
        ...
        # Break the loop after the first frame to only process one
        print("Stopping after one frame as requested.")
        break
